...
6. Confirme que **Ativos cotados**, **Cotações disponíveis**, **Fontes cotando** e **Último dado** são derivados apenas das fontes conectadas e do conteúdo exibido. Feche um MT5 e confirme que sua fonte e suas cotações saem imediatamente do Dashboard, embora o último snapshot continue acessível no **Diagnóstico**. O Dashboard não deve mostrar força, tendência, notícias ou qualquer análise ainda inexistente; os módulos futuros continuam apenas como **Em breve**.
7. Provoque apenas estados reais seguros, como MT5 fechado, aguardando login e corretora desconectada. Confirme que o cabeçalho distingue `N conectada(s)` de `capacidade MT5: A de L`, sem usar o total de cadastros como denominador, e que os badges em **Terminais MT5** mostram os detalhes.
...
Use a instalação de teste sincronizada e mantenha o Gerenciador de Tarefas disponível para confirmar os processos.
...
9. Confirme no Gerenciador de Tarefas que os workers Python e cada `terminal64.exe` controlado terminaram realmente.
10. Inicie o aplicativo sem abrir MT5, feche pelo X e confirme encerramento imediato, sem erro e sem segundo efeito ao clicar novamente.
11. Após reabrir o aplicativo, confirme que cadastros, seleção, aliases e fluxos preservados continuam coerentes; nenhum sinal atrasado deve restaurar **Conectado** ou **MT5 aberto** para um processo já encerrado.
12. Repita fechamento individual e em lote com os outros MT5 ativos e confirme isolamento: fechar um terminal não interrompe leitura nem processo dos demais antes de eles próprios entrarem no lote.
13. Não provoque `terminate()`/`kill()` em conta real. A escalada resistente continua validada somente pelos fakes automatizados.
