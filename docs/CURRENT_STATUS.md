...
- DEV-008A disponibilizou o núcleo causal genérico do Atlas. A DEV-008B.1A acrescenta o materializador offline de sessões M1 encerradas, com manifesto estrito, objetos imutáveis por sessão, correção histórica, recuperação e promoção por `current.json`; execução real e integração diária permanecem em portões separados — ver `docs/ATLAS_INCREMENTAL.md`.
- DEV-008B.1B implementa e testa o adaptador incremental de ticks encerrados de contratos individuais para M1, sem unir vencimentos. M1 e gerações são imutáveis, o relógio fornecido pelo MT5 é preservado sem conversão adicional e o manifesto resultante alimenta a DEV-008B.1A. A execução real corrigida do `WINV26` promoveu 11 sessões na geração `20260916T192118686687_633cd170`; a repetição comprovou `no_change`. A materialização dessas sessões no Atlas permanece no próximo portão — ver `docs/ATLAS_TICK_ADAPTER.md`.
