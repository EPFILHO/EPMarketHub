...
- DEV-008B.1B implementa e testa o adaptador incremental de ticks encerrados de contratos individuais para M1, sem unir vencimentos. M1 e gerações são imutáveis, o relógio fornecido pelo MT5 é preservado sem conversão adicional e o manifesto resultante alimenta a DEV-008B.1A. A primeira execução real do `WINV26` processou 11 sessões e a auditoria pós-execução corrigiu uma conversão indevida de três horas; a geração corrigida deve ser reconstruída antes do portão Atlas — ver `docs/ATLAS_TICK_ADAPTER.md`.
