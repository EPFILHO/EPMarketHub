...
- DEV-008B.1B implementa e testa o adaptador incremental de ticks encerrados de contratos individuais para M1, sem unir vencimentos. M1 e gerações são imutáveis, o relógio de sessão é normalizado explicitamente e o manifesto resultante alimenta a DEV-008B.1A. O manifesto local do `WINV26` está preparado, mas nenhum tick real foi processado — ver `docs/ATLAS_TICK_ADAPTER.md`.
